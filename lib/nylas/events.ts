 ...
 * Everything else requires a manual pin via the tutor calendar.
 */
const AUTO_COUNT_RE = /\btutor(?:ing)?\b|\[tutoring\]|\bsat\s*prep\b/i;

/** Returns true if the title auto-matches as a tutoring session. */
function titleMatchesTutoring(title: string): boolean {
  return AUTO_COUNT_RE.test(title);
}
